...
class Engine(object):
    def __init__(self, state={}):
        ...
        self.state["max_iterations"] = self.state["max_epochs"] * int(
            (self.state["images_num"] / self.state["batch_size"]) + 1
        )
    ...
    def train(self, data_loader, model, criterion, optimizer, epoch):
        ...
        for i, (input, target) in enumerate(data_loader):
            ...
            self.state["iteration"] = i + epoch * int(
                (self.state["images_num"] / self.state["batch_size"]) + 1
            )
        ...
        if training:
            ...
            with torch.no_grad():
                with open("out.csv", "a") as f:
                    ...
                    f.write(
                        to_csv(self.state["target"].detach().cpu().numpy()[0]) + ","
                    )
                ...
                with open("out.csv", "a") as f:
                    f.write(
                        to_csv(self.state["output"].detach().cpu().numpy()[0]) + "\n"
                    )
