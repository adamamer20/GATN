...
class GATNResnet(nn.Module):
    def __init__(self, model_name, num_classes, in_channel=300, t1=0.0, adj_file=None):
        ...
        # Topology
        # s_adj1 = gen_A(num_classes, 1.0, t1, adj_file)
        s_adj = gen_A_correlation(
            num_classes, 1.0, 0.4, "data/topology/yelp_correlation_adj.pkl"
        )
        s_adj1 = gen_A_correlation(
            num_classes, 0.4, 0.2, "data/topology/yelp_correlation_adj.pkl"
        )

        """if num_classes == 20:
            s_adj = gen_A_correlation(num_classes, 1.0, 0.4, "GATN/model/topology/voc_correlation_adj.pkl")
            s_adj1 = gen_A_correlation(num_classes, 0.4, 0.2, "GATN/model/topology/voc_correlation_adj.pkl")
        else:
            s_adj = gen_A_correlation(num_classes, 1.0, 0.2, "GATN/model/topology/coco_correlation_adj.pkl")
            s_adj1 = gen_A_correlation(num_classes, 0.4, 0.2, "GATN/model/topology/coco_correlation_adj.pkl")
    ...
    def forward(self, feature, inp, label=None, iters=1, max_iters=256600):
        ...
        adj = (
            torch.squeeze(adj, 0)
            + torch.eye(self.num_classes).type(torch.FloatTensor).cuda()
        )
        adj = gen_adj(adj)
    ...
    def get_config_optim(self, lr, lrp, lrt):
        ...
        config_optim.append(
            {"params": self.transformerblock.parameters(), "lr": lr * lrt}
        )
